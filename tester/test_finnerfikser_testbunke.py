...
REFUSJON = "Refusjon utbetales til kontonummer: 60111000002"


def test_merket_kontonummer_stjeles_ikke_av_fnr_presedensen():
    """60111000002 består BEGGE sjekksummene (gyldig som D-nummer). Uten
    etikettvakt ble refusjonskontoen rapportert som FØDSELSNUMMER og
    manglet helt i kontonummerlista."""
    assert "60111000002" in finn_alle_kontonummer(REFUSJON)
    assert "60111000002" not in finn_alle_fodselsnummer(REFUSJON)
...
def test_umerket_dobbeltgyldig_beholder_fnr_presedens():
    ...
    tekst = "verdien 60111000002 står her uten etikett"
    assert "60111000002" in finn_alle_fodselsnummer(tekst)
    assert finn_alle_kontonummer(tekst) == []


def test_ekte_fnr_med_etikett_uendret():
    tekst = "Foedselsnummer: 14837840059"
    assert finn_alle_fodselsnummer(tekst) == ["14837840059"]
    assert finn_alle_kontonummer(tekst) == []
