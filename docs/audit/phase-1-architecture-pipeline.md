...
| Kjent punkt (6): én FAILED side blokkerer `/tekst` for alltid | **Var reelt — nå fikset og bevist på BLANDET tilstand:** 3-siders dokument der side 1 tvinges FAILED mens 0 og 2 er DONE → `/tekst` gir HTTP 200, `ferdig=true`, `komplett=false`, `feilede_sider=[1]`, sidene 0 og 2 leveres med tekst, side 1 med `status=FAILED, tekst=null`. `/felter` gir `state=DONE_MED_FEILEDE_SIDER`, `beslutning=REVIEW` (feilet side = ukjent innhold → tvunget til gjennomgang), `sider_til_gjennomgang=[1]`. Én ødelagt side stopper altså IKKE de andre. | bekreftet ved kjøring (blandet DONE/FAILED, ikke bare alle-FAILED) |
