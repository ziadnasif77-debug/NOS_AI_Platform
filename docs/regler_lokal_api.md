...
| R66 | **Dokumentprofilen er obligatorisk.** `POST /dokument` returnerer ALLTID `dokumentprofil`, uavhengig av hva klienten ba om og i begge kontraktene (brytere og `operasjoner`): `antall_sider` (filens FAKTISKE sider, ikke sider med tekst), QR-/strekkodesider, `dokumentdato` + `dokumentdato_fra`/`til` + `dokument_ar` + `dokument_alder`, `stempel_datoer`, `dokument_eier` og `ytelse` (plassholder). Alt er deterministisk — ingen modell. Et dokument har egenskaper som gjelder uansett spørsmål; måtte de bes om enkeltvis, ville de bli glemt. | KODE |
