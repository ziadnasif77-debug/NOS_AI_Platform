...
| R69 | **Fødselsnummeret tilhører den dokumentet GJELDER.** Hvert nummer kobles til etiketten som står nærmest FORAN det: `Dokumentet gjelder`/`Navn`/`Søker`/`Bruker`/`Den sykmeldte`/`Pasient` gir eier; `Saksbehandler`/`Lege`/`Arbeidsgiver`/`Kopi`/`Signert av`/`Verge` gir en annen rolle, og et slikt nummer blir ALDRI dokumentets. Uten et positivt eiersignal er svaret `null` — et nummer hentes ikke bare fordi det står i teksten. `sikkerhet` sier hvorfor (`merket`/`flertydig`/`bare_andre_roller`/`umerket`/`ingen`), og `kandidater` viser alt som ble vurdert. | KODE |
